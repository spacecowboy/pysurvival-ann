                                                  ...
                                                  boost::variate_generator<boost::mt19937&, boost::uniform_int<> > *uniform)
{
  ...
  net->mutateWeights(gaussian, uniform, cloner->weightMutationChance,
                     cloner->weightMutationStdDev, 1, 0);
...
double GeneticSurvivalNetwork::getDecayL1() const {
	 return decayL1;
}
void GeneticSurvivalNetwork::setDecayL1(double val) {
	 this->decayL1 = val;
}

double GeneticSurvivalNetwork::getDecayL2() const {
	 return decayL2;
}
void GeneticSurvivalNetwork::setDecayL2(double val) {
	 this->decayL2 = val;
}

double GeneticSurvivalNetwork::getWeightElimination() const {
	 return weightElimination;
}
void GeneticSurvivalNetwork::setWeightElimination(double val) {
	 this->weightElimination = val;
}
double GeneticSurvivalNetwork::getWeightEliminationLambda() const {
	 return weightEliminationLambda;
}
void GeneticSurvivalNetwork::setWeightEliminationLambda(double val) {
...
double weightSquaredSum(GeneticSurvivalNetwork *net) {
	 double sum = 0;
...
double weightAbsoluteSum(GeneticSurvivalNetwork *net) {
	 double sum = 0;
	 unsigned int n, numOfCons = 0;
	 for (n = 0; n < net->getNumOfHidden(); n++) {
		  // neuron weights
...
double weightEliminationSum(GeneticSurvivalNetwork *net, double lambda) {
	 double sum = 0;
	 unsigned int n, numOfCons = 0;
	 for (n = 0; n < net->getNumOfHidden(); n++) {
		  // neuron weights
...
evaluateNetwork(GeneticSurvivalNetwork *net, double *X, double *Y,
...
void GeneticSurvivalNetwork::learn(double *X, double *Y,
  ...
  for (i = 0; i < populationSize + 1; i++) {
       GeneticSurvivalNetwork *net = getGeneticSurvivalNetwork(this, &gaussian,
                                                               &uniform);
    // evaluate error here
