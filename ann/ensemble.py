...
class Ensemble(object):
    '''Holds a list of any ANNs. Learn() uses bagging to give each network a
    unique data set. Bagging can be disabled by setting the attribute, then
    each network is trained on the entire dataset.
    ...
    def __getattr__(self, name):
        ...
        Any method or property whose name starts with an underscore '_'. This
        is due to python's special handling of those methods and wrapping them
        is unwanted, it breaks pickling and who know's what else.
        """
    ...
    def _learn_bagged(self, trndata, targets, *args, **kwargs):
        ...
        for net in self.networks:
            ...
            baggeddata = bagging(np.column_stack([trndata, targets]),
                                 count=self.bagging_limit)
            tl = len(targets[0])
            tc = list(reversed([(-1 - x) for x in range(tl)]))
            result.append(net.learn(baggeddata[:, :-tl],
                                    np.column_stack([baggeddata[:, tc]]),
                                    *args, **kwargs))
        return np.array(result)
